...
class ComparisonController(QObject):
    ...
    def __init__(self):
        ...
        self.widgets = [
            self.fixedImageWidget,
            self.diffImageWidget,
            self.movingImageWidget,
        ]
    ...
    def setInputData(self, fixedDataName, movingDataName, transform=None):
        ...
        transformedData = transformer.TransformImageData(
            movingImageData, self.transform, fixedImageData
        )
        ...
        # fixedWorldSize = [fixedBounds[1] - fixedBounds[0], fixedBounds[3]
        #      - fixedBounds[2], fixedBounds[5] - fixedBounds[4]]
        ...
        # fixedSubExtents[0] = int((fixedDimensions[0]-1) * ((bsubBounds[0]
        #       - fixedBounds[0]) / fixedWorldSize[0]))
        # fixedSubExtents[1] = int((fixedDimensions[0]-1) * ((subBounds[1]
        #       - fixedBounds[0]) / fixedWorldSize[0]))
        # fixedSubExtents[2] = int((fixedDimensions[1]-1) * ((subBounds[2]
        #       - fixedBounds[2]) / fixedWorldSize[1]))
        # fixedSubExtents[3] = int((fixedDimensions[1]-1) * ((subBounds[3]
        #       - fixedBounds[2]) / fixedWorldSize[1]))
        # fixedSubExtents[4] = int((fixedDimensions[2]-1) * ((subBounds[4]
        #       - fixedBounds[4]) / fixedWorldSize[2]))
        # fixedSubExtents[5] = int((fixedDimensions[2]-1) * ((subBounds[5]
        #       - fixedBounds[4]) / fixedWorldSize[2]))
        ...
        self.diffImageWidget.setSlicerWidget(
            self.fixedImageWidget, self.movingImageWidget
        )
        ...
        self.fixedImageWidget.slicer.AddObserver(
            "WindowLevelEvent", self.slicerModified
        )
        self.movingImageWidget.slicer.AddObserver(
            "WindowLevelEvent", self.slicerModified
        )
