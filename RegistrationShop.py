...
class RegistrationShop(MainWindow, WindowDialog):
    ...
    def createElements(self):
        ...
        self.movingPropWidget = RenderPropWidget(
            self.movingRenderController, parent=self
        )
    ...
    def createActions(self):
        ...
        self.actionFreeTransformTool = QAction(
            "Manual transform", self, shortcut="Ctrl+1"
        )
        ...
        self.actionLandmarkTransformTool = QAction(
            "Landmark transform", self, shortcut="Ctrl+2"
        )
        ...
        self.actionDeformableTransformTool = QAction(
            "Automatic transform", self, shortcut="Ctrl+3"
        )
        ...
        self.actionLoadFixedData = QAction(
            "Load fixed data", self, shortcut="Ctrl+Shift+F"
        )
        self.actionLoadFixedData.triggered.connect(self.loadFixedDataSetFile)

        self.actionLoadMovingData = QAction(
            "Load moving data", self, shortcut="Ctrl+Shift+M"
        )
        ...
        self.actionSaveProjectAs = QAction(
            "Save project as...", self, shortcut="Ctrl+Shift+S"
        )
    ...
    def createToolbar(self):
        ...
        statusWidget.setText(
            "Welcome to RegistrationShop!\nStart your registration by loading two "
            "datasets. After that you can use the transform tools to align your volume "
            "data."
        )
    ...
    def addManualTransform(self):
        ...
        * this would mean a new data model for the multi render widget
        ...
        self.transformTool.setRenderWidgets(
            moving=self.movingDataWidget, multi=self.multiDataWidget
        )
    ...
    def addLandmarkTransform(self):
        if not self.fixedDataWidget.imageData or not self.movingDataWidget.imageData:
            statusWidget = StatusWidget.Instance()
            statusWidget.setText(
                "Please load a fixed and a moving dataset before starting a landmark "
                "transform."
            )
            return
    ...
    def addDeformableTransform(self):
        if not self.fixedDataWidget.imageData or not self.movingDataWidget.imageData:
            statusWidget = StatusWidget.Instance()
            statusWidget.setText(
                "Please load a fixed and a moving dataset before starting a deformable "
                "transform."
            )
            return
