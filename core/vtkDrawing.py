...
def CreateSquare(width, color=None, zOffset=0):
    ...
    squareSource.SetBounds(
        -halfWidth, halfWidth, -halfWidth, halfWidth, zOffset, zOffset
    )
...
def CreateBounds(bounds):
    ...
    lineActors = []
    lineActors += CreateLineBeginAndEnd(
        [originX, originY, originZ], [boundX, originY, originZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [originX, originY, originZ], [originX, boundY, originZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [originX, originY, originZ], [originX, originY, boundZ], linePartLength
    )
    ...
    lineActors += CreateLineBeginAndEnd(
        [boundX, boundY, boundZ], [boundX, boundY, originZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [boundX, boundY, boundZ], [originX, boundY, boundZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [boundX, boundY, boundZ], [boundX, originY, boundZ], linePartLength
    )

    lineActors += CreateLineBeginAndEnd(
        [boundX, originY, originZ], [boundX, originY, boundZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [boundX, originY, originZ], [boundX, boundY, originZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [originX, boundY, originZ], [originX, boundY, boundZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [originX, boundY, originZ], [boundX, boundY, originZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [originX, originY, boundZ], [originX, boundY, boundZ], linePartLength
    )
    lineActors += CreateLineBeginAndEnd(
        [originX, originY, boundZ], [boundX, originY, boundZ], linePartLength
    )
...
def CreateOrientationGrid(bounds, camera):
    ...
    for index in range(1, int(boundX / subdivSize)):
        ...
        lineActorsX.append(
            CreateLine([index * subdivSize, 0, 0], [index * subdivSize, 0, handleSize])
        )
    ...
    for index in range(1, int(boundY / subdivSize)):
        handleSize = smallHandleSize if index % 5 != 0 else bigHandleSize
        lineActorsY.append(
            CreateLine([0, index * subdivSize, 0], [handleSize, index * subdivSize, 0])
        )
        lineActorsY.append(
            CreateLine([0, index * subdivSize, 0], [0, index * subdivSize, handleSize])
        )
        if index > 0 and index % 5 == 0:
            textItem = CreateTextItem(str(index * subdivSize), 0.4 * subdivSize, camera)
            textItem.SetPosition(
                [-smallHandleSize, index * subdivSize, -smallHandleSize]
            )
            ColorActor(textItem, color=[0.6, 0.6, 0.6])
            lineText.append(textItem)
    ...
    for index in range(1, int(boundZ / subdivSize)):
        handleSize = smallHandleSize if index % 5 != 0 else bigHandleSize
        lineActorsZ.append(
            CreateLine([0, 0, index * subdivSize], [handleSize, 0, index * subdivSize])
        )
        lineActorsZ.append(
            CreateLine([0, 0, index * subdivSize], [0, handleSize, index * subdivSize])
        )
        if index > 0 and index % 5 == 0:
            textItem = CreateTextItem(str(index * subdivSize), 0.4 * subdivSize, camera)
