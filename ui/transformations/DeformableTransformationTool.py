...
class DeformableTransformationTool(TransformationTool):
    ...
    def setRenderWidgets(self, fixed, moving, multi):
        ...
        statusWidget.setText(
            "Adjust the parameters for the transform. See the Elastix "
    ...
    def applyTransform(self):
        ...
        * Load the new data into the moving widget / project
        ...
        for filename in filenames:
            if (
                os.path.isdir(os.path.join(dataFolder, filename))
                and "result" in filename
            ):
                resultId += 1
        ...
        command = ElastixCommand(
            fixedData=currentProject.fixedData,
        ...
        if os.path.exists(outputData):
            statusWidget.setText(
                "Thanks for your patience. The transformed data will now "
                "be loaded. It can be found in the project folder."
            )

            transformation = Transformation(
                vtkTransform(), Transformation.TypeDeformable, outputData
            )
