...
class TwoStepPicker(Picker):
    ...
    def setPropertiesWidget(self, widget):
        ...
        self.propertiesWidget.histogramWidget.updatedPosition.connect(
            self.histogramUpdatedPosition
        )
        self.locatorUpdated.connect(
            self.propertiesWidget.histogramWidget.locatorUpdated
        )
    ...
    def mouseMove(self, iren, event=""):
        ...
        if iren.GetShiftKey() != 0:
            ...
            penalty = [
                (sampleSize + 1) / float(1 + abs(i - sampleSize))
    ...
    def _setLine(self, point1, point2):
        ...
        sortedIntersections = sortedLocations(
            intersections[0], intersections[1], self.camPosition()
        )
        ...
        self.lineActorOverlay = CreateLine(
            sortedIntersections[0], sortedIntersections[1]
        )
        ...
        abLength = Length(ab)
        nrOfSteps = (
            256  # TODO: make this number dependent on data size and length of vector
        )
        ...
        for i in range(nrOfSteps + 1):
            ...
            self.samples.append(
                interpolator.Interpolate(sampleLoc[0], sampleLoc[1], sampleLoc[2], 0)
            )
            # Update the sampling position
            sampleLoc = Add(sampleLoc, abStep)

        self.propertiesWidget.setSamples(
            self.samples, self.widget.imageData.GetScalarRange()
        )
