...
class VolumeVisualizationWrapper(object):
    ...
    def setVolumeVisualization(self, volumeVisualization):
        for attribute in VolumeVisualizationWrapper.standardAttributes:
            if (
                hasattr(volumeVisualization, attribute)
                and getattr(volumeVisualization, attribute) is not None
            ):
        ...
        if hasattr(volumeVisualization, "opacityFunction"):
            self.opacityFunction = vtkPiecewiseFunctionWrapper(
                volumeVisualization.opacityFunction
            )
        if hasattr(volumeVisualization, "colorFunction"):
            self.colorFunction = vtkColorTransferFunctionWrapper(
                volumeVisualization.colorFunction
            )

    def getVolumeVisualization(self):
        volumeVisualization = VolumeVisualizationFactory.CreateProperty(
