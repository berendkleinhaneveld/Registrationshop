...
from .VolumeVisualization import VisualizationTypeMIDA, VolumeVisualization
...
class VolumeVisualizationMIDA(VolumeVisualization):
    ...
    def updateTransferFunction(self):
        self.colorFunction, self.opacityFunction = CreateRangeFunctions(
            self.minimum, self.maximum, self.window, self.level
        )
        self.volProp.SetColor(self.colorFunction)
        self.volProp.SetScalarOpacity(self.opacityFunction)

        if self.mapper:
            lowerBound = (self.lowerBound - self.minimum) / (
                self.maximum - self.minimum
            )
            upperBound = (self.upperBound - self.minimum) / (
                self.maximum - self.minimum
            )
