...
class TransferFunctionWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.histogramWidget.setAxeMode(
            bottom=HistogramWidget.AxeClear, left=HistogramWidget.AxeLog
        )
    ...
    def updateNodes(self):
        for index in range(len(self.transferFunction.points)):
            ...
            nodeItem.setPosition(
                [
                    (point.value - self.range[0]) / (self.range[1] - self.range[0]),
                    point.opacity,
                ]
            )
