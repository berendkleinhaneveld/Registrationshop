...
class MultiRenderWidget(QWidget):
    ...
    * colorized mix mode
    ...
    def _updateGrids(self):
        ...
        for item in (
            self.movingGridItems + self.fixedGridItems + self.orientationGridItems
        ):
            self.renderer.AddViewProp(item)

    def _cleanUpGrids(self):
        for item in (
            self.fixedGridItems + self.movingGridItems + self.orientationGridItems
        ):
    ...
    def _updateMapper(self, volVis, volNr):
        ...
        if volNr == 1:
            ...
            if shaderType == 2:  # MIDA
                lowerBound = (volVis.lowerBound - volVis.minimum) / (
                    volVis.maximum - volVis.minimum
                )
                upperBound = (volVis.upperBound - volVis.minimum) / (
                    volVis.maximum - volVis.minimum
                )
