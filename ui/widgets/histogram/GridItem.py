...
class GridItem(QGraphicsRectItem):
    ...
    def paint(self, painter, option, widget):
        ...
        height = self.rect().height() - (
            self._margins.top() + self._margins.bottom() + 1
        )
        ...
        for i in range(len(self._itemsLeft)):
            ...
            if self._margins.left() > 2:
                ...
                painter.drawText(
                    QRectF(0, (-0.5 * 10) + yPos, xOffset - 3, 10),
        ...
        for i in range(len(self._itemsBottom)):
            ...
            if len(self._itemsBottom[i]) > 0:
                ...
                if i > 0 and i < len(self._itemsBottom) - 1:
                    painter.drawLine(
                        QPointF(xPos, yOffset), QPointF(xPos, height + yOffset)
                    )
                painter.drawText(
                    QRectF(xPos - (0.5 * xOffset), height + 10, xOffset, 10),
                    Qt.AlignCenter,
                    self._itemsBottom[i],
                )
